...
const Forecast: React.FC = () => {
  return (
    <section id="forecast" className="py-16 bg-market-dark">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 gap-8">
          <Card className="p-8 bg-market-darkblue shadow-md border border-white/10">
            <h3 className="text-2xl font-bold mb-6 text-gray-100">Q2 2025 Forecast Highlights</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              <div className="text-center p-4 bg-market-gradient1/40 rounded-lg border border-white/10">
                <h4 className="font-medium text-gray-300">Median Price Forecast</h4>
                <p className="text-2xl font-bold text-market-blue mt-2">$440K - $455K</p>
                <p className="text-sm text-gray-400 mt-1">+3.5% expected by Q3</p>
              </div>
              <div className="text-center p-4 bg-market-gradient1/40 rounded-lg border border-white/10">
                <h4 className="font-medium text-gray-300">Inventory Forecast</h4>
                <p className="text-2xl font-bold text-market-blue mt-2">1,100 - 1,300</p>
                <p className="text-sm text-gray-400 mt-1">Slight seasonal increase</p>
              </div>
              <div className="text-center p-4 bg-market-gradient1/40 rounded-lg border border-white/10">
                <h4 className="font-medium text-gray-300">Mortgage Rate Forecast</h4>
                <p className="text-2xl font-bold text-market-blue mt-2">5.8% - 6.2%</p>
                <p className="text-sm text-gray-400 mt-1">Stabilizing after Q1 volatility</p>
              </div>
            </div>
            
            <h3 className="text-xl font-bold mb-4 mt-8 text-gray-100">Expert Insights</h3>
              ...
              {expertComments.map((comment, index) => (
                <div key={index} className="space-y-2">
                  <div className="flex flex-wrap items-baseline justify-between gap-2">
                    <h4 className="font-semibold text-lg text-gray-200">{comment.name}, <span className="font-normal text-gray-400">{comment.role}</span></h4>
                    <span className="text-sm text-gray-400">{comment.date}</span>
                  </div>
                  <p className="text-gray-300">{comment.comment}</p>
                  {index < expertComments.length - 1 && <Separator className="mt-6 bg-white/10" />}
                </div>
              ))}
            </div>
          </Card>
